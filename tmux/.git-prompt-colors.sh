...
override_git_prompt_colors() {
  GIT_PROMPT_THEME_NAME="Custom"          # Jellybeans
  ...
  GIT_PROMPT_STAGED="${White}●"            # the number of staged files/directories
  GIT_PROMPT_CONFLICTS="${Red}✖"         # the number of files in conflict
  GIT_PROMPT_CHANGED="${White}✚"          # the number of changed files

  GIT_PROMPT_REMOTE=" "                 # the remote branch name (if any) and the symbols for ahead and behind
  GIT_PROMPT_UNTRACKED="${White}…"       # the number of untracked files/dirs
  GIT_PROMPT_STASHED="${BoldWhite}⚑"    # the number of stashed files/dir
  GIT_PROMPT_CLEAN="${BoldWhite}✔"      # a colored flag indicating a "clean" repo
  ...
  GIT_PROMPT_START_USER="${ResetColor}${Green}${USER}@${HOSTNAME%%.*}${ResetColor} ${BoldBlue}${PathShort}${ResetColor}"
